...
type position = {
  ...
  list_ind : card_lst_ind;
  (**index of the card list*)

  card_ind : card_ind_in_lst;
...
module PlayerHand = struct
  ...
  let is_empty (hand : card list) = function [] -> true | _ -> false
  ...
  let new_pos_info (c : card) (loc : deck_player_table) (cli : card_lst_ind) 
  (ci : card_ind_in_lst)
  ...
  (**[update_tail_card_lst] increment the card index of the cards in the tail 
  card list by 1. helper function for [card_inset]*)
  let rec update_tail_card_lst (cl : card list) = 
    match cl with 
    | [] -> []
    | h :: t -> (new_card h 
    (new_pos_info h h.pos.location h.pos.list_ind (h.pos.card_ind + 1))) 
    :: update_tail_card_lst t

  (**[card_insert] inserts [c] to a card list [cl] based on [c]'s postion*)
  let rec card_insert (c : card) (cl : card list) = 
    match cl with 
    | [] -> [c]
    | h :: t -> if h.pos = c.pos then c :: (update_tail_card_lst cl)
    else h :: card_insert c t


  (**[update_card_position] updates the card's position bassed on user choice.
  It returns a new card with the old card information but new postion. 
  The card will be appended to the destination card list*)
  let update_card_position (c : card) (p : position) (cl : card list)= 
    {color = c.color; number = c.number; index = c.index; pos = p} :: cl
